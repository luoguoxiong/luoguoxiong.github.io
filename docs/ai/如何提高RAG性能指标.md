...
召回上来了，不代表答案就对。准确率包含两层：**检索的精准度**（Context Precision）和**生成的忠实度**（Faithfulness）。
...
1. **召回率是天花板**：先解决「找得到」，再谈「答得对」。切分和混合检索是最被低估的优化点
2. **准确率靠两层**：重排序解决检索精准度，Prompt 约束和引用溯源解决生成忠实度
3. **速度主战场在 LLM**：检索环节已接近极限，真正的延迟大头在生成环节。流式输出 + Cheap-First + 缓存，见效最快

永远记住：**先建评估基线，再动手优化。** 没有指标驱动，一切优化都是盲人摸象。
